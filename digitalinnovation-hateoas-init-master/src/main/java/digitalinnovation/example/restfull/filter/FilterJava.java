...
public class FilterJava implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        logger.info("Início da chamada do filtro - método doFilter da classe FilterJava");
        HttpServletRequest req = (HttpServletRequest) request;
        Enumeration<String> headerNames = req.getHeaderNames();
        Map<String, String> mapHeaders = Collections.list(headerNames)
                .stream()
                .collect(Collectors.toMap(it -> it, req::getHeader));
                // pega a estrutura key, value que veio com a reuisição
                // e joga para um mapa
        ...
        logger.info("Final da chamada do filtro - método doFilter da classe FilterJava");
    }
}
